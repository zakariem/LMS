    ...
    public partial class viewStudent : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                using (SqlConnection conn = connectionClass.connect())
                {
                    SqlCommand cmd = new SqlCommand("update Student set sName = @sName, entroll = @entroll, depart = @depart, sem = @sem, contact = @contact,email = @email", conn);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            else if (MessageBox.Show("are you sure?", "Checking", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                using (SqlConnection conn = connectionClass.connect())
                {
                    SqlCommand cmd = new SqlCommand("delete from student where sName = @sName", conn);
                    cmd.Parameters.AddWithValue("@sName", txtSname.Text);
